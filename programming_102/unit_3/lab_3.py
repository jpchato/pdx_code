...
'''
def units_to_meters():
    conversion_dict = {'feet': 0.3048, 'mile': 1609.34, 'kilometer': 1000, 'meter': 1}
    units = input('Which unit do you need to convert to meters? feet, mile, kilometer, or meter? ')
    length = input ('How many units do you need to convert? ')
    answer = conversion_dict.get(units)
    print('answer: ' + str(answer))
    print('length: ' + str(length))
    total = int(answer) * int(length)
